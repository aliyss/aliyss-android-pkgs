...
def test_fdroid_category_map_covers_entire_index_taxonomy():
    ...
    taxonomy = [
        "System", "Internet", "Multimedia", "Science & Education", "Connectivity",
        "Security", "Sports & Health", "Reading", "Writing", "Navigation",
        "Keyboard & IME", "Development", "Note", "Phone & SMS", "Graphics",
        "Task", "Puzzle Game", "Timer", "Game Helper", "Local Media Player",
        "Calendar & Agenda", "Finance Manager", "Password & 2FA", "Online Media Player",
        "Messaging", "VPN & Proxy", "Wallpaper", "Launcher", "Remote Controller",
        "Health Manager", "Translation & Dictionary", "News", "Calculator",
        "Board Game", "Music Practice Tool", "Workout", "Social Network",
        "Cloud Storage & File Sync", "Market & Price", "Camera", "Habit Tracker",
        "Wallet", "Religion", "Ebook Reader", "File Transfer", "App Manager",
        "Educational Game", "Weather", "Time Tracker", "Action Game", "Alarm Clock",
        "Notification", "Download", "Location Tracker & Sharer", "Forum", "Clock",
        "Schedule", "AI Chat", "Voice & Video Chat", "File Encryption & Vault",
        "Casual Game", "Network Analyzer", "Public Transport", "Card Game", "Draw",
        "Recorder", "Email", "Diet", "Podcast", "DNS & Hosts", "Gallery",
        "Strategy Game", "Role-Playing Game", "Browser", "Dice", "Firewall",
        "Shopping List", "Radio", "Text Editor", "Cast", "App Store & Updater",
        "Inventory", "Word Game", "Meditation", "Bookmark", "Remote Access",
        "Icon Pack", "Volume", "File Manager", "Battery", "Shooter Game",
        "Unit Convertor", "Recipe Manager", "Stopwatch", "Party Game", "Emulator",
        "Flashlight", "Code & Forge", "OCR", "Push", "Text to Speech", "Pass Wallet",
        "Lyrics", "Contact", "Platformer Game", "Sport Game", "Theming",
        "Visual Novel", "Time", "Xposed", "Office", "Automation", "Food", "Games",
    ]
    ...
    assert unmapped <= broad_by_keywords, f"unmapped index categories: {unmapped - broad_by_keywords}"
...
def test_category_from_index_uses_curated_first():
    assert cat.category_from_index("com.foo", ["Internet", "Messaging"], "chat", "Foo") == "messaging"
...
def test_category_from_index_prefers_distinctive_of_several():
    ...
    assert cat.category_from_index(
        "xyz.ptgms.tosdr", ["Internet", "Navigation", "Reading", "Security"],
        "ToS summaries", "ToS;DR") == "security"
    assert cat.category_from_index(
        "com.grocery.app", ["Diet", "Shopping List", "Sports & Health"]) == "health"
...
def test_category_from_index_falls_back_to_keywords_for_broad_buckets():
    ...
    assert cat.category_from_index("com.foo.bar", ["Internet"], "a fast web browser", "Foo") == "browser"
    assert cat.category_from_index("com.foo.bar", ["Multimedia"], "offline music player", "Foo") == "music"
