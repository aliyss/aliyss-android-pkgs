...
def _make_pkgs(tmp_path: Path) -> Path:
    pkgs = tmp_path / "pkgs"
    for cat, apps in [
        ("misc", ["com.foo.app", "com.bar.browser", "com.unknown.zzz"]),
...
def _cli(pkgs_dir: Path, file: str | None, **kw: Any) -> Any:
    defaults: dict[str, Any] = dict(
        repo="fdroid", file=file, dry_run=True, all=False, pkgs_dir=pkgs_dir
    )
    defaults.update(kw)
...
def test_all_considers_non_misc_dirs(tmp_path):
    ...
    moves_default = recategorize.plan_moves(
        pkgs, recategorize.metadata_map(index), all_categories=False
    )
